  ...
  return (
    <div className="w-[40%] ">
      <div className={`rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.15)] p-10 flex flex-col items-center justify-cente ${!dark ? 'bg-[#444444]' : "bg-[#D9D9D9]"} `}>
        <h1 className={`font-poppins font-bold text-[50px] ${!dark ? 'text-white' : ''}`}>
          {
          details && details?.name }</h1>

        <div>
          <h1 className={`font-poppins font-bold text-[70px] pt-5 ${!dark ? 'text-white' : ''}`}>{locationData && time()}</h1>
          <h2 className={`font-poppins ${!dark ? 'text-white text-center' : ''}`}>{locationData && date()}</h2>
        </div>
      </div>
